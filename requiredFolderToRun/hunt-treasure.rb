...
class Player
    ...
    def explore_room 
        # Encounters 
        if (encounterCallee(:guard) or encounterCallee(:pit) or  encounterCallee(:bats) )
    ...
    def executeActions(playerAction) 
        if (playerAction.to_s=="move")
            puts "Chicken"
        end 
    end
    #Player's action 
